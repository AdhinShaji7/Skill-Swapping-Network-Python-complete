...
def admin_managebadge():
	...
	if "action" in request.args:
		...
		action=None
	if action=='delete':
		de="delete from badgeorachive where badgeorachive_id='%s'"%(cid)
		delete(de)
		return redirect(url_for('admin.admin_managebadge'))
	if action=='update':
		up="select * from badgeorachive where badgeorachive_id='%s'"%(cid)
		data['catupdate']=select(up)
		if "sub1" in request.form:
			fn=request.form['category']
			upd="update badgeorachive set badgeorchive='%s' where badgeorachive_id='%s'"%(fn,cid)
			update(upd)
			return redirect(url_for('admin.admin_managebadge'))
